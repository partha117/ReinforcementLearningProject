...
def ppo_step(policy_net, value_net, optimizer_policy, optimizer_value, optim_value_iternum, states, actions,
             returns, advantages, fixed_log_probs, clip_epsilon, l2_reg, batch_action, batch_hidden):
    """update critic"""
    for _ in range(optim_value_iternum):
        values_pred, _ = value_net(states, actions=batch_action, hidden=batch_hidden)
...
def update_params(samples, value_net, policy_net, policy_optimizer, value_optimizer, gamma, tau, device, l2_reg,
    ...
    batch_picked = torch.tensor(
        ...
        torch.float)
    with torch.no_grad():
        values, _ = value_net(state, actions=batch_picked, hidden=batch_hidden)
    ...
    for _ in range(optim_epochs):
        ...
        states, actions, returns, advantages, fixed_log_probs, already_picked, hidden_states = \
            state[perm].clone(), action[perm].clone(), returns[perm].clone(), advantages[perm].clone(), \
            fixed_log_probs[perm].clone(), batch_picked[perm].clone(), batch_hidden[:, :, perm, :].clone()

        for i in range(optim_iter_num):
            ind = slice(i * optim_batch_size, min((i + 1) * optim_batch_size, states.shape[0]))
            states_b, actions_b, advantages_b, returns_b, fixed_log_probs_b, already_picked_b, hidden_states_b = \
