...
def a2c_step(policy_net, value_net, optimizer_policy, optimizer_value, states, actions, returns, advantages, l2_reg, batch_picked, batch_hidden):
    """update critic"""
    values_pred, hidden_pred = value_net(states, actions=batch_picked, hidden=batch_hidden)
...
def update_params(samples, value_net, policy_net, policy_optimizer, value_optimizer, gamma, tau, device, l2_reg):
    ...
    batch_picked = torch.tensor(
        ...
        torch.float)
    with torch.no_grad():
        values, _ = value_net(state.type(torch.float).to(device),actions=batch_picked, hidden=batch_hidden)

    """get advantage estimation from the trajectories"""
    advantages, returns = estimate_advantages(reward, done, values.view(-1,1), gamma, tau, device)

    """perform TRPO update"""
    a2c_step(policy_net, value_net, policy_optimizer, value_optimizer, state.type(torch.float).to(device), action.to(device), returns, advantages, l2_reg, batch_picked, batch_hidden)
